...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    torchrun --nproc_per_node=$num_gpu vibe/bin/train.py \
        ...
        --gpu $gpus \
        --train_manifest /home/jovyan/workspace/vibe/manifests/cnceleb_dev.jsonl \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    torchrun --nproc_per_node=$num_gpu vibe/bin/extract.py \
        --exp_dir $exp_dir \
        --audio_scp /home/jovyan/workspace/vibe/manifests/cnceleb1.scp \
        --use_gpu \
        --gpu $gpus
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    trials="$trials_dir/cnceleb1_trial.txt"
