...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    torchrun --nproc_per_node=$num_gpu vibe/bin/train.py \
        --config conf/ecapa_tdnn.yaml \
        --gpu $gpus \
        --train_manifest /home/jovyan/workspace/vibe/manifests/voxceleb2_train.jsonl \
        --validation_manifest /home/jovyan/workspace/vibe/manifests/voxceleb2_validation.jsonl \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    num_gpu=$(echo $gpus | awk -F ' ' '{print NF}')
    torchrun --nproc_per_node=$num_gpu vibe/bin/extract.py \
        --exp_dir $exp_dir \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    trials="$trials_dir/voxceleb1_o_trial.txt $trials_dir/voxceleb1_e_trial.txt $trials_dir/voxceleb1_h_trial.txt"
