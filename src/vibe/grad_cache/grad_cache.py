...
class GradCache:
    """
    Gradient Cache class. Implements input chunking, first graph-less forward pass, 
    Gradient Cache creation, second forward & backward gradient computation. 
    
    Optimizer step is not included. Native torch automatic mixed precision is supported. 
    User needs to handle gradient unscaling and scaler update after a gradient cache step.
    """
    def __init__(
        self,
        ...
        get_rep_fn: Optional[Callable[..., torch.Tensor]] = None,
        accelerator = None,
    ):
        """
        ...
        Args:
            models: A list of all encoder models to be updated by the current cache.
            chunk_sizes: An integer indicating chunk size. Or a list of integers of chunk size for each model.
            loss_fn: A loss function that takes arbitrary numbers of representation tensors and
                arbitrary numbers of keyword arguments as input. It should not in any case modify 
                the input tensors' relations in the autograd graph, which are later relied upon 
                to create the gradient cache.
            split_input_fn: An optional function that split generic model input into chunks. 
                If not provided, this class will try its best to split the inputs of supported types.
                See `split_inputs` function.
            get_rep_fn: An optional function that takes generic model output and return 
                representation tensors. If not provided, the generic output is assumed to be 
                the representation tensor.
            accelerator: Optional accelerator for backward pass.
        """
        self.models = models
    ...
    def split_inputs(self, model_input, chunk_size: int) -> List:
        """
        Split input into chunks. Will call user provided `split_input_fn` if specified.
        Otherwise, it can handle input types of tensor, list of tensors and dictionary of tensors.
        
        Args:
            model_input: Generic model input.
            chunk_size: Size of each chunk.
            
        Returns:
            A list of chunked model input.
        """
    ...
    def get_input_tensors(self, model_input) -> List[torch.Tensor]:
        """
        Recursively go through model input and grab all tensors, which are then used 
        to record current device random states.
        
        This method will do its best to parse types of Tensor, tuple, list, dict and UserDict.
        Other types will be ignored unless self._get_input_tensors_strict is set to True, 
        ...
        Returns:
            All torch tensors in model_input
        """
    ...
    def model_call(self, model: nn.Module, model_input):
        ...
        Args:
            ...
            model_input: Input to the model call
        ...
        with nullcontext():
            if isinstance(model_input, torch.Tensor):
                ...
                raise NotImplementedError(f"Unsupported model_input type: {type(model_input)}")

    def get_reps(self, model_out) -> torch.Tensor:
        """
        Return representation tensor from generic model output.
        
        Args:
            model_out: Generic model output
            
        Returns:
            A single tensor corresponding to the model representation output
        """
    ...
    def compute_loss(self, *reps: torch.Tensor, **loss_kwargs) -> torch.Tensor:
        """
        Compute the loss based on the representation tensors. The tensors should be 
        ordered same as the list of models registered in this GradCache class instance.
        
        Args:
            reps: Representations for computing the loss.
            loss_kwargs: Keyword arguments input to the loss function.
    ...
    def forward_no_grad(
        ...
        Args:
            model: Encoder model.
            model_inputs: Model input already broken into chunks.
            
        Returns:
            A tuple of (representations, recorded_random_states).
        """
        rnd_states = []
        model_reps = []
    ...
    def build_cache(self, *reps: torch.Tensor, **loss_kwargs) -> Tuple[List[torch.Tensor], torch.Tensor]:
        """
        Compute the gradient cache.
        
        Args:
            reps: Computed representations from all encoder models
            loss_kwargs: Extra keyword arguments to the loss function
            
        Returns:
            A tuple of (gradient_cache_for_each_encoder_model, loss_tensor)
        """
    ...
    def forward_backward(
        ...
        model: nn.Module,
        model_inputs,
        cached_gradients: List[torch.Tensor],
        ...
        Args:
            model: Encoder model.
            model_inputs: Chunked input to the encoder model.
            cached_gradients: Chunked gradient cache tensor for each input.
            random_states: Each input's device random state during the first forward.
            no_sync_except_last: If True, under distributed setup, only trigger gradient 
                reduction across processes for the last sub-batch's forward-backward pass.
        """
    ...
    def cache_step(
        ...
        **loss_kwargs
        ...
        Args:
            model_inputs: Input to each encoder model. Should be in similar order as the class's model.
            no_sync_except_last: If True, under distributed setup, for each model, only trigger 
                gradient reduction across processes for the last sub-batch's forward-backward pass.
            loss_kwargs: Additional keyword arguments to the loss function.
            
        Returns:
            The current step loss.
        """
        all_reps = []
        all_rnd_states = []
        ...
        if no_sync_except_last:
            assert all(map(lambda m: isinstance(m, nn.parallel.DistributedDataParallel), self.models)), \
                'Some models are not wrapped in DistributedDataParallel. Make sure you are running DDP with ' \
                'proper initializations.'

        # Split inputs for each model according to their chunk sizes
        model_inputs = [
            self.split_inputs(x, chunk_size) 
            for x, chunk_size in zip(model_inputs, self.chunk_sizes)
        ]

        # First forward pass without gradients
        ...
        # Second forward pass and backward with gradients
