...
from .configuration_campplus import CAMPPlusConfig
...
def get_nonlinear(config_str, channels):
    """Create a sequential module of nonlinear operations based on config string.
    
    Args:
        config_str: A string of hyphen-separated nonlinear operation names
        channels: Number of channels for operations that need it
        
    Returns:
        nn.Sequential: A sequence of nonlinear operations
    """
...
class TDNNLayer(nn.Module):
    ...
    A 1D convolutional layer with additional nonlinear operations.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            padding: Padding size. If negative, computed based on kernel size
            dilation: Dilation rate of the convolution
            bias: Whether to include bias parameters
            config_str: Configuration string for nonlinear operations
        """
...
class CAMLayer(nn.Module):
    ...
    This layer applies a context-aware mask to the local convolution output.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            dilation: Dilation rate of the convolution
            bias: Whether to include bias parameters
            reduction: Reduction factor for channel dimension in the bottleneck
        """
...
class CAMDenseTDNNLayer(nn.Module):
    ...
    This layer combines nonlinear operations with a CAM layer.
    """
    ...
    def __init__(
        ...
        """Initialize the CAMDenseTDNN layer."""
    ...
    def bn_function(self, x):
        """Apply bottleneck function to input."""
...
class CAMDenseTDNNBlock(nn.ModuleList):
    """Block of multiple CAMDenseTDNN layers with dense connections."""
    ...
    def __init__(
        ...
        """Initialize the CAMDenseTDNN block."""
    ...
    def forward(self, x):
        """Forward pass through CAMDenseTDNN block with dense connectivity."""
        for layer in self:
...
class TransitLayer(nn.Module):
    ...
    Applies nonlinear operations followed by channel reduction.
    """
...
class DenseLayer(nn.Module):
    """Dense layer with nonlinear operations.
    
    Applies a 1x1 convolution followed by nonlinear operations.
    """
...
class BasicResBlock(nn.Module):
    """Basic residual block for the Feature Convolution Module.
...
class FCM(nn.Module):
    """Feature Convolution Module.
    
    Processes features using 2D convolutions and residual blocks.
    """
    ...
    def forward(self, x):
        """Forward pass through Feature Convolution Module."""
...
class CAMPPlus(torch.nn.Module):
    ...
    A fast and efficient network using Context-Aware Masking.
    """
    ...
    def __init__(
        ...
        # Create dense blocks with different configurations
        block_configs = [
        ...
        for i, (num_layers, kernel_size, dilation) in enumerate(block_configs):
            ...
            # Update channel count after dense connectivity
        ...
        self.asp_bn = BatchNorm1d(input_size=channels * 2)
    ...
    def get_frame_level_feat(self, x):
        ...
        Returns:
            Frame-level features of shape (batch_size, time_steps, dimension)
        """
        x = x.permute(0, 2, 1)  # (B,T,F) => (B,F,T)
        x = self.head(x)
        
        # Apply all layers except the last two
        for layer in self.xvector[:-2]:
            x = layer(x)
        
        # Return to original dimension order
        out = x.permute(0, 2, 1)  # (B,F,T) => (B,T,F)
        return out
...
class CAMPPlusForSpeakerClassification(nn.Module, PyTorchModelHubMixin):

    def __init__(self, config: CAMPPlusConfig):
        super().__init__()
        self.config = config
        self.num_classes = config.num_labels
        self.mean_var_norm = InputNormalization(std_norm=False)
        self.encoder = CAMPPlus(
            feat_dim=config.num_mel_bins, 
